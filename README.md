...
这是一个基于 PySide6 的桌面工具，用于读取多种实验数据文件，预览并合并数据列，并支持将结果导出或发送到 Origin。根目录的 `数据合并工具_v2.3.0.py` 是本地启动入口，实际业务代码在 `src/data_merge_tool/` 中。

## 目录结构

- `src/data_merge_tool/`：当前 v2.3.0 模块化源码。
...
- `origin_client.py` / `origin_worker.py` / `origin_automation.py`：Origin 自动化子进程通信、入口和 worker 专用实现。
